...
namespace TaleWorlds.Core
{
	...
	public sealed class SiegeEngineType : MBObjectBase
	{
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			this.Description = new TextObject(node.Attributes.get_ItemOf("description").InnerText, null);
			XmlAttribute xmlAttribute = node.Attributes.get_ItemOf("max_hit_points");
			...
			XmlAttribute xmlAttribute2 = node.Attributes.get_ItemOf("difficulty");
			this.Difficulty = Convert.ToInt32((xmlAttribute2 != null) ? xmlAttribute2.Value : null);
			this.ToolCost = Convert.ToInt32(node.Attributes.get_ItemOf("tool_cost").Value);
			this.HitChance = (float)Convert.ToDouble(node.Attributes.get_ItemOf("hit_chance").Value);
			this.IsAntiPersonnel = Convert.ToBoolean(node.Attributes.get_ItemOf("is_anti_personnel").Value);
			this.AntiPersonnelHitChance = (this.IsAntiPersonnel ? ((float)Convert.ToDouble(node.Attributes.get_ItemOf("anti_personnel_hit_chance").Value)) : 0f);
			this.IsConstructible = Convert.ToBoolean(node.Attributes.get_ItemOf("is_constructible").Value);
			this.IsRanged = Convert.ToBoolean(node.Attributes.get_ItemOf("is_ranged").Value);
			this.Damage = Convert.ToInt32(node.Attributes.get_ItemOf("damage").Value);
			this.ManDayCost = Convert.ToInt32(node.Attributes.get_ItemOf("man_day_cost").Value);
			this.CampaignRateOfFirePerDay = (float)Convert.ToDouble(node.Attributes.get_ItemOf("campaign_rate_of_fire_per_day").Value);
			this.MovementSpeed = (float)Convert.ToDouble(node.Attributes.get_ItemOf("movement_speed").Value);
			this.ProjectileSpeed = (float)Convert.ToDouble(node.Attributes.get_ItemOf("projectile_speed").Value);
		}
	}
}
