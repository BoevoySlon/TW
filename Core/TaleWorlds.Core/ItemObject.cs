...
namespace TaleWorlds.Core
{
	...
	public sealed class ItemObject : MBObjectBase
	{
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			{
				XmlNode xmlNode = node.Attributes.get_ItemOf("multiplayer_item");
				if (xmlNode != null && !string.IsNullOrEmpty(xmlNode.InnerText))
				{
					this.MultiplayerItem = (xmlNode.InnerText == "true");
				}
				XmlNode xmlNode2 = node.Attributes.get_ItemOf("is_merchandise");
				if (xmlNode2 != null && !string.IsNullOrEmpty(xmlNode2.InnerText))
				{
					this.NotMerchandise = (xmlNode2.InnerText != "true");
				}
				TextObject weaponName = new TextObject(node.Attributes.get_ItemOf("name").InnerText, null);
				string innerText = node.Attributes.get_ItemOf("crafting_template").InnerText;
				bool flag = node.Attributes.get_ItemOf("has_modifier") == null || node.Attributes.get_ItemOf("has_modifier").InnerText != "false";
				...
				{
					if (node.ChildNodes.get_ItemOf(i).Name == "Pieces")
					{
						xmlNode3 = node.ChildNodes.get_ItemOf(i);
						break;
					}
				}
				foreach (object obj in xmlNode3.ChildNodes)
				{
					...
					{
						XmlNode xmlNode5 = xmlNode4.Attributes.get_ItemOf("id");
						XmlAttribute xmlAttribute2 = xmlNode4.Attributes.get_ItemOf("Type");
						XmlAttribute xmlAttribute3 = xmlNode4.Attributes.get_ItemOf("scale_factor");
				...
				if (node.Attributes.get_ItemOf("value") != null)
				{
					this.Value = int.Parse(node.Attributes.get_ItemOf("value").Value);
				}
				else
				{
					this.DetermineValue();
				...
				this.PrerequisiteItem = ((node.Attributes.get_ItemOf("prerequisite_item") != null) ? ((ItemObject)objectManager.ReadObjectReferenceFromXml("prerequisite_item", typeof(ItemObject), node)) : null);
			}
			else
			{
				this.Name = new TextObject(node.Attributes.get_ItemOf("name").InnerText, null);
				XmlNode xmlNode6 = node.Attributes.get_ItemOf("multiplayer_item");
				if (xmlNode6 != null && !string.IsNullOrEmpty(xmlNode6.InnerText))
				{
					this.MultiplayerItem = (xmlNode6.InnerText == "true");
				}
				XmlNode xmlNode7 = node.Attributes.get_ItemOf("is_merchandise");
				if (xmlNode7 != null && !string.IsNullOrEmpty(xmlNode7.InnerText))
				{
					this.NotMerchandise = (xmlNode7.InnerText != "true");
				}
				this.PrerequisiteItem = ((node.Attributes.get_ItemOf("prerequisite_item") != null) ? ((ItemObject)objectManager.ReadObjectReferenceFromXml("prerequisite_item", typeof(ItemObject), node)) : null);
				XmlNode xmlNode8 = node.Attributes.get_ItemOf("mesh");
				if (xmlNode8 != null && !string.IsNullOrEmpty(xmlNode8.InnerText))
				{
					this.MultiMeshName = xmlNode8.InnerText;
				}
				this.HolsterMeshName = ((node.Attributes.get_ItemOf("holster_mesh") != null) ? node.Attributes.get_ItemOf("holster_mesh").Value : null);
				this.HolsterWithWeaponMeshName = ((node.Attributes.get_ItemOf("holster_mesh_with_weapon") != null) ? node.Attributes.get_ItemOf("holster_mesh_with_weapon").Value : null);
				this.FlyingMeshName = ((node.Attributes.get_ItemOf("flying_mesh") != null) ? node.Attributes.get_ItemOf("flying_mesh").Value : null);
				this.HasLowerHolsterPriority = false;
				if (node.Attributes.get_ItemOf("item_holsters") != null)
				{
					this.ItemHolsters = node.Attributes.get_ItemOf("item_holsters").Value.Split(new char[]
					{
						':'
					});
					if (node.Attributes.get_ItemOf("has_lower_holster_priority") != null)
					{
						this.HasLowerHolsterPriority = bool.Parse(node.Attributes.get_ItemOf("has_lower_holster_priority").Value);
					}
				}
				else
				{
				...
				this.HolsterPositionShift = ((node.Attributes.get_ItemOf("holster_position_shift") != null) ? Vec3.Parse(node.Attributes.get_ItemOf("holster_position_shift").Value) : Vec3.Zero);
				this.BodyName = ((node.Attributes.get_ItemOf("body_name") != null) ? node.Attributes.get_ItemOf("body_name").Value : null);
				this.HolsterBodyName = ((node.Attributes.get_ItemOf("holster_body_name") != null) ? node.Attributes.get_ItemOf("holster_body_name").Value : null);
				this.CollisionBodyName = ((node.Attributes.get_ItemOf("shield_body_name") != null) ? node.Attributes.get_ItemOf("shield_body_name").Value : null);
				this.RecalculateBody = (node.Attributes.get_ItemOf("recalculate_body") != null && bool.Parse(node.Attributes.get_ItemOf("recalculate_body").Value));
				XmlNode xmlNode9 = node.Attributes.get_ItemOf("prefab");
				...
				this.Culture = (BasicCultureObject)objectManager.ReadObjectReferenceFromXml("culture", typeof(BasicCultureObject), node);
				string text2 = (node.Attributes.get_ItemOf("item_category") != null) ? node.Attributes.get_ItemOf("item_category").Value : null;
				if (!string.IsNullOrEmpty(text2))
				{
					this.ItemCategory = Game.Current.ObjectManager.GetObject<ItemCategory>(text2);
				}
				this.Weight = ((node.Attributes.get_ItemOf("weight") != null) ? float.Parse(node.Attributes.get_ItemOf("weight").Value) : 1f);
				this.LodAtlasIndex = ((node.Attributes.get_ItemOf("lod_atlas_index") != null) ? int.Parse(node.Attributes.get_ItemOf("lod_atlas_index").Value) : -1);
				XmlAttribute xmlAttribute4 = node.Attributes.get_ItemOf("difficulty");
				if (xmlAttribute4 != null)
				{
					this.Difficulty = int.Parse(xmlAttribute4.Value);
				}
				XmlAttribute xmlAttribute5 = node.Attributes.get_ItemOf("appearance");
				this.Appearance = ((xmlAttribute5 != null) ? float.Parse(xmlAttribute5.Value) : 0.5f);
				XmlAttribute xmlAttribute6 = node.Attributes.get_ItemOf("IsFood");
				if (xmlAttribute6 != null)
				{
					this.IsFood = Convert.ToBoolean(xmlAttribute6.InnerText);
				}
				this.IsUsingTableau = (node.Attributes.get_ItemOf("using_tableau") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("using_tableau").InnerText));
				XmlNode xmlNode10 = node.Attributes.get_ItemOf("using_arm_band");
				if (xmlNode10 != null)
				{
					this.ArmBandMeshName = Convert.ToString(xmlNode10.InnerText);
				}
				this.ScaleFactor = ((node.Attributes.get_ItemOf("scale_factor") != null) ? float.Parse(node.Attributes.get_ItemOf("scale_factor").Value) : 1f);
				...
				foreach (object obj2 in node.ChildNodes)
				{
					...
					if (xmlNode11.Name == "Flags")
					{
						foreach (object obj4 in Enum.GetValues(typeof(ItemFlags)))
						{
							...
							XmlAttribute xmlAttribute7 = xmlNode11.Attributes.get_ItemOf(itemFlags.ToString());
				...
				XmlAttribute xmlAttribute8 = node.Attributes.get_ItemOf("Type");
				...
				XmlAttribute xmlAttribute9 = node.Attributes.get_ItemOf("AmmoOffset");
