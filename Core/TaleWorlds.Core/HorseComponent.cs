...
namespace TaleWorlds.Core
{
	...
	public class HorseComponent : ItemComponent
	{
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			XmlAttribute xmlAttribute = node.Attributes.get_ItemOf("skeleton_scale");
			...
			foreach (object obj in node.ChildNodes)
			{
				...
				if (xmlNode.Name == "Materials")
				{
					using (IEnumerator enumerator2 = xmlNode.ChildNodes.GetEnumerator())
					{
						while (enumerator2.MoveNext())
						{
							...
							{
								string value = xmlNode2.Attributes.get_ItemOf("name").Value;
								...
								foreach (object obj3 in xmlNode2.ChildNodes)
								{
									...
									if (xmlNode3.Name == "MeshMultipliers")
									{
										foreach (object obj4 in xmlNode3.ChildNodes)
										{
											...
											{
												uint item = (xmlNode4.Attributes.get_ItemOf("mesh_multiplier") != null) ? Convert.ToUInt32(xmlNode4.Attributes.get_ItemOf("mesh_multiplier").Value, 16) : 0U;
												float item2 = (xmlNode4.Attributes.get_ItemOf("percentage") != null) ? ((float)Convert.ToDouble(xmlNode4.Attributes.get_ItemOf("percentage").Value)) : 0f;
				...
				if (xmlNode.Name == "AdditionalMeshes")
				{
					foreach (object obj5 in xmlNode.ChildNodes)
					{
						...
						if (xmlNode5.Name == "Mesh" && xmlNode5.Attributes.get_ItemOf("name") != null)
						{
							bool value2 = false;
							if (xmlNode5.Attributes.get_ItemOf("affected_by_cover") != null)
							{
								value2 = bool.TryParse(xmlNode5.Attributes.get_ItemOf("affected_by_cover").InnerText, out value2);
							}
							this.AdditionalMeshesNameList.Add(new KeyValuePair<string, bool>(xmlNode5.Attributes.get_ItemOf("name").InnerText, value2));
						}
					}
				}
			}
		...
		{
			// Token: 0x06000A05 RID: 2565 RVA: 0x00020B03 File Offset: 0x0001ED03
			...
			// (get) Token: 0x06000A06 RID: 2566 RVA: 0x00020B17 File Offset: 0x0001ED17
			// (set) Token: 0x06000A07 RID: 2567 RVA: 0x00020B1F File Offset: 0x0001ED1F
