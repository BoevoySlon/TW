...
namespace TaleWorlds.CampaignSystem
{
	...
	public sealed class Kingdom : MBObjectBase, IFaction
	{
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			this.EncyclopediaTitle = ((node.Attributes.get_ItemOf("title") != null) ? new TextObject(node.Attributes.get_ItemOf("title").Value, null) : TextObject.Empty);
			this.EncyclopediaRulerTitle = ((node.Attributes.get_ItemOf("ruler_title") != null) ? new TextObject(node.Attributes.get_ItemOf("ruler_title").Value, null) : TextObject.Empty);
			this.InitializeKingdom(new TextObject(node.Attributes.get_ItemOf("name").Value, null), (node.Attributes.get_ItemOf("short_name") != null) ? new TextObject(node.Attributes.get_ItemOf("short_name").Value, null) : new TextObject(node.Attributes.get_ItemOf("name").Value, null), (CultureObject)objectManager.ReadObjectReferenceFromXml("culture", typeof(CultureObject), node), null, (node.Attributes.get_ItemOf("color") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("color").Value, 16), (node.Attributes.get_ItemOf("color2") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("color2").Value, 16), null, this.EncyclopediaText, this.EncyclopediaTitle, this.EncyclopediaRulerTitle);
			...
			this.AlternativeColor = ((node.Attributes.get_ItemOf("alternative_color") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("alternative_color").Value, 16));
			this.AlternativeColor2 = ((node.Attributes.get_ItemOf("alternative_color2") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("alternative_color2").Value, 16));
			this.PrimaryBannerColor = ((node.Attributes.get_ItemOf("primary_banner_color") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("primary_banner_color").Value, 16));
			this.SecondaryBannerColor = ((node.Attributes.get_ItemOf("secondary_banner_color") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("secondary_banner_color").Value, 16));
			if (node.Attributes.get_ItemOf("banner_key") != null)
			{
				this.Banner = new Banner();
				this.Banner.Deserialize(node.Attributes.get_ItemOf("banner_key").Value);
			}
			else
			{
			...
			foreach (object obj in node.ChildNodes)
			{
				...
				if (xmlNode.Name == "relationships")
				{
					using (IEnumerator enumerator2 = xmlNode.ChildNodes.GetEnumerator())
					{
						while (enumerator2.MoveNext())
						{
							...
							if (xmlNode2.Attributes.get_ItemOf("isAtWar") != null && Convert.ToBoolean(xmlNode2.Attributes.get_ItemOf("isAtWar").Value))
							{
								FactionManager.DeclareWar(this, faction, false);
							}
						}
				...
				if (xmlNode.Name == "policies")
				{
					foreach (object obj3 in xmlNode.ChildNodes)
					{
						...
						PolicyObject @object = Game.Current.ObjectManager.GetObject<PolicyObject>(xmlNode3.Attributes.get_ItemOf("id").Value);
						if (@object != null)
						{
