...
namespace TaleWorlds.CampaignSystem
{
	...
	public sealed class Clan : MBObjectBase, IFaction
	{
		...
		{
			if (MBSaveLoad.IsUpdatingGameVersion && MBSaveLoad.LastLoadedGameVersion < ApplicationVersion.FromString("v1.2.2", 66233) && base.StringId == "neutral")
			{
		...
		{
			this.UpdateStrength();
			if (MBSaveLoad.IsUpdatingGameVersion && MBSaveLoad.LastLoadedGameVersion < ApplicationVersion.FromString("e1.8.0.0", 66233) && this.Kingdom != null)
			{
				FactionHelper.AdjustFactionStancesForClanJoiningKingdom(this, this.Kingdom);
			}
			if (MBSaveLoad.IsUpdatingGameVersion && MBSaveLoad.LastLoadedGameVersion < ApplicationVersion.FromString("v1.1.3", 66233) && this.Kingdom == null && this.IsUnderMercenaryService)
			{
				this.EndMercenaryService(true);
			}
			if (MBSaveLoad.IsUpdatingGameVersion && MBSaveLoad.LastLoadedGameVersion < ApplicationVersion.FromString("v1.2.0", 66233) && this.IsEliminated && this.Leader != null && this.Leader.IsAlive)
			{
				DestroyClanAction.Apply(this);
			}
		}
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			this.Tier = ((node.Attributes.get_ItemOf("tier") == null) ? 1 : Convert.ToInt32(node.Attributes.get_ItemOf("tier").Value));
			this.Renown = (float)Campaign.Current.Models.ClanTierModel.CalculateInitialRenown(this);
			this.InitializeClan(new TextObject(node.Attributes.get_ItemOf("name").Value, null), (node.Attributes.get_ItemOf("short_name") != null) ? new TextObject(node.Attributes.get_ItemOf("short_name").Value, null) : new TextObject(node.Attributes.get_ItemOf("name").Value, null), (CultureObject)objectManager.ReadObjectReferenceFromXml("culture", typeof(CultureObject), node), null, default(Vec2), true);
			XmlNode xmlNode = node.Attributes.get_ItemOf("is_noble");
			if (xmlNode != null)
			{
				this.IsNoble = Convert.ToBoolean(xmlNode.InnerText);
			}
			this.LabelColor = ((node.Attributes.get_ItemOf("label_color") == null) ? 0U : Convert.ToUInt32(node.Attributes.get_ItemOf("label_color").Value, 16));
			this.Color = ((node.Attributes.get_ItemOf("color") == null) ? 4291609515U : Convert.ToUInt32(node.Attributes.get_ItemOf("color").Value, 16));
			this.Color2 = ((node.Attributes.get_ItemOf("color2") == null) ? 4291609515U : Convert.ToUInt32(node.Attributes.get_ItemOf("color2").Value, 16));
			this.AlternativeColor = ((node.Attributes.get_ItemOf("alternative_color") == null) ? 4291609515U : Convert.ToUInt32(node.Attributes.get_ItemOf("alternative_color").Value, 16));
			this.AlternativeColor2 = ((node.Attributes.get_ItemOf("alternative_color2") == null) ? 4291609515U : Convert.ToUInt32(node.Attributes.get_ItemOf("alternative_color2").Value, 16));
			if (node.Attributes.get_ItemOf("initial_posX") != null && node.Attributes.get_ItemOf("initial_posY") != null)
			{
				this.InitialPosition = new Vec2((float)Convert.ToDouble(node.Attributes.get_ItemOf("initial_posX").Value), (float)Convert.ToDouble(node.Attributes.get_ItemOf("initial_posY").Value));
			}
			this.IsBanditFaction = (node.Attributes.get_ItemOf("is_bandit") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_bandit").Value));
			this.IsMinorFaction = (node.Attributes.get_ItemOf("is_minor_faction") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_minor_faction").Value));
			this.IsOutlaw = (node.Attributes.get_ItemOf("is_outlaw") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_outlaw").Value));
			this.IsSect = (node.Attributes.get_ItemOf("is_sect") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_sect").Value));
			this.IsMafia = (node.Attributes.get_ItemOf("is_mafia") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_mafia").Value));
			this.IsClanTypeMercenary = (node.Attributes.get_ItemOf("is_clan_type_mercenary") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_clan_type_mercenary").Value));
			this.IsNomad = (node.Attributes.get_ItemOf("is_nomad") != null && Convert.ToBoolean(node.Attributes.get_ItemOf("is_nomad").Value));
			...
			foreach (object obj in node.ChildNodes)
			{
				...
				{
					IFaction faction;
					if (xmlNode2.Attributes.get_ItemOf("clan") != null)
					{
					...
					int num = Convert.ToInt32(xmlNode2.Attributes.get_ItemOf("value").InnerText);
					if (num > 0)
					{
