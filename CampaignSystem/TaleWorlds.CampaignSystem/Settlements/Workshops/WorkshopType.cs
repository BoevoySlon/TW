...
namespace TaleWorlds.CampaignSystem.Settlements.Workshops
{
	...
	public sealed class WorkshopType : MBObjectBase
	{
		...
		public override void Deserialize(MBObjectManager objectManager, XmlNode node)
		{
			...
			this.JobName = new TextObject(node.Attributes.get_ItemOf("jobname").Value, null);
			this.Description = new TextObject(node.Attributes.get_ItemOf("description").Value, null);
			this.EquipmentCost = int.Parse(node.Attributes.get_ItemOf("equipment_cost").Value);
			this.Frequency = ((node.Attributes.get_ItemOf("frequency") != null) ? int.Parse(node.Attributes.get_ItemOf("frequency").Value) : 1);
			this.IsHidden = (node.Attributes.get_ItemOf("isHidden") != null && bool.Parse(node.Attributes.get_ItemOf("isHidden").Value));
			...
			foreach (object obj in node.ChildNodes)
			{
				...
				if (xmlNode.Name == "Production")
				{
					...
					float conversionSpeed = (xmlNode.Attributes.get_ItemOf("conversion_speed") != null) ? float.Parse(xmlNode.Attributes.get_ItemOf("conversion_speed").Value) : 0f;
					...
					foreach (object obj2 in xmlNode)
					{
						...
						if (xmlNode2.Name == "Inputs")
						{
							foreach (object obj3 in xmlNode2)
							{
								...
								{
									string value = xmlNode3.Attributes.get_ItemOf(0).Value;
									if (!string.IsNullOrEmpty(value))
									{
										ItemCategory @object = objectManager.GetObject<ItemCategory>(value);
										int count = (xmlNode3.Attributes.get_ItemOf("input_count") != null) ? int.Parse(xmlNode3.Attributes.get_ItemOf("input_count").Value) : 1;
										if (@object != null)
										{
						...
						if (xmlNode2.Name == "Outputs")
						{
							foreach (object obj4 in xmlNode2)
							{
								...
								if (xmlNode4.Attributes.Count > 0)
								{
									...
									int outputCount = (xmlNode4.Attributes.get_ItemOf("output_count") != null) ? int.Parse(xmlNode4.Attributes.get_ItemOf("output_count").Value) : 1;
				...
				else if (xmlNode.Name == "Meshes")
				{
					...
					for (int i = 1; i < 4; i++)
					{
						...
						if (xmlNode.Attributes.get_ItemOf(text) != null)
						{
							this.PropMeshName3List.Add(xmlNode.Attributes.get_ItemOf(text).Value);
						}
					}
